...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['add_event'])) {
            ...
            // Get existing images
            $stmt = $pdo->prepare("SELECT image_paths FROM events WHERE id = ?");
            $stmt->execute([$event_id]);
            $existing = $stmt->fetch();
            $existing_images = $existing && $existing['image_paths'] ? json_decode($existing['image_paths'], true) : [];
            ...
            if (!empty($_FILES['images']['name'][0])) {
                ...
                foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
                    if ($_FILES['images']['error'][$key] === UPLOAD_ERR_OK) {
                        ...
                        $target_path = $upload_dir . $filename;
                        if (move_uploaded_file($tmp_name, $target_path)) {
                            $new_images[] = "/LIFT-Kompass/" . $target_path;
                        }
                    }
                }
            }
            ...
            $stmt->execute([$date, $title, $description, json_encode($all_images), $event_id]);
            ...
            $stmt = $pdo->prepare("INSERT INTO issues (title, description, image_paths) VALUES (?, ?, ?)");
            $stmt->execute([$title, $description, json_encode($carousel_images)]);
            ...
            $stmt = $pdo->prepare("UPDATE issues SET title = ?, description = ?, image_paths = ? WHERE id = ?");
            $stmt->execute([$title, $description, json_encode($all_images), $issue_id]);
            ...
            // Get existing photos
            $stmt = $pdo->prepare("SELECT photos FROM team_year_posts WHERE id = ?");
            $stmt->execute([$post_id]);
            $existing = $stmt->fetch();
            $existing_photos = $existing && $existing['photos'] ? json_decode($existing['photos'], true) : [];
            ...
            // Get existing images
            $stmt = $pdo->prepare("SELECT image_paths FROM lesekreis WHERE id = ?");
            $stmt->execute([$lesekreis_id]);
            $existing = $stmt->fetch();
            $existing_images = $existing && $existing['image_paths'] ? json_decode($existing['image_paths'], true) : [];
            if (!is_array($existing_images)) $existing_images = [];
            ...
            $stmt->execute([$date, $title, $description, json_encode($all_images), $lesekreis_id]);
...
$issues = $pdo->query("SELECT id, title, description, image_paths FROM issues ORDER BY id DESC")->fetchAll();
...
if (isset($_GET['edit_issue'])) {
    $stmt = $pdo->prepare("SELECT id, title, description, image_paths FROM issues WHERE id = ?");
...
<head>
    ...
    <style>
        ...
        .section { margin: 30px 0; padding: 20px; border: 1px solid #3471B8; border-radius: 10px; }
        .form-group { margin: 10px 0; }
        .form-group label { display: block; margin-bottom: 5px; font-weight: bold; }
        .form-group input, .form-group textarea, .form-group select { width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 5px; box-sizing: border-box; }
        .form-group textarea { height: 100px; }
        button { background-color: #3471B8; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin: 5px; }
        .edit-btn { background-color: #28a745; }
        .cancel-btn { background-color: #6c757d; }
        .logout-btn { background-color: #dc3545; text-decoration: none; padding: 10px 20px; border-radius: 5px; color: white; display: inline-block; }
        .message { padding: 10px; background-color: #d4edda; color: #155724; border-radius: 5px; margin: 10px 0; }
        ...
        .tab.active { background-color: #3471B8; color: white; }
        .tab-content { display: none; }
        .tab-content.active { display: block; }
        .current-images { margin: 10px 0; padding: 10px; background: #f8f9fa; border-radius: 5px; }
        .current-images img { max-width: 100px; margin: 5px; border: 1px solid #ddd; }
    </style>
</head>
<body>
<div class="admin-container">
    ...
    <div id="events" class="tab-content <?= !isset($_GET['edit_issue']) && !isset($_GET['edit_press']) && !isset($_GET['edit_member_post']) && !isset($_GET['edit_lesekreis']) ? 'active' : '' ?>">
        <div class="section">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <?php endif; ?>
                <div class="form-group"><label>New Images (optional):</label><input type="file" name="images[]" multiple accept="image/*"></div>
                <?php if ($edit_event): ?>
                    <button type="submit" name="update_event">Update Event</button><a href="?" class="cancel-btn" style="text-decoration: none; display: inline-block;">Cancel</a>
                <?php else: ?>
                ...
                <?php foreach ($events as $event): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="POST" style="display:inline;"><input type="hidden" name="event_id" value="<?= $event['id'] ?>"><button type="submit" name="delete_event" onclick="return confirm('Delete this event?')">Delete</button></form>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </table>
                ...
                <?php if ($edit_issue && isset($edit_issue['image_paths']) && $edit_issue['image_paths']): ?>
                    <div class="current-images"><strong>Current Carousel Images:</strong><br>
                        <?php $images = json_decode($edit_issue['image_paths'], true); if (is_array($images)): foreach ($images as $image): ?>
                            <div style="display: inline-block; margin:5px;"><img src="<?= htmlspecialchars($image) ?>" style="max-width:100px;"><br><small><?= basename($image) ?></small></div>
                        <?php endforeach; endif; ?>
                    </div>
                <?php endif; ?>
                <div class="form-group"><label>Carousel Images (optional, multiple):</label><input type="file" name="carousel_images[]" multiple accept="image/*"><small>These images will be displayed in the carousel on the main page.</small></div>
                <?php if ($edit_issue): ?>
                    <button type="submit" name="update_issue">Update Issue</button><a href="?" class="cancel-btn" style="text-decoration: none; display: inline-block;">Cancel</a>
                <?php else: ?>
                ...
                <?php foreach ($issues as $issue): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="POST" style="display:inline;"><input type="hidden" name="issue_id" value="<?= $issue['id'] ?>"><button type="submit" name="delete_issue" onclick="return confirm('Delete this issue?')">Delete</button></form>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </table>
    ...
    <div id="press" class="tab-content <?= isset($_GET['edit_press']) ? 'active' : '' ?>">
        ...
        <div class="section">
            ...
            <table>
                <tr><th>ID</th><th>Source</th><th>Title</th><th>URL</th><th>Actions</th></tr>
                <?php foreach ($press_mentions as $press): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="POST" style="display:inline;"><input type="hidden" name="press_id" value="<?= $press['id'] ?>"><button type="submit" name="delete_press" onclick="return confirm('Delete this press mention?')">Delete</button></form>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </table>
    ...
    <div id="members" class="tab-content <?= isset($_GET['edit_member_post']) ? 'active' : '' ?>">
        <div class="section">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group"><label>Additional Text (e.g., Gründer):</label><textarea name="additional_text" rows="3" placeholder="Enter additional text with names"><?= (isset($edit_member_post) && $edit_member_post) ? htmlspecialchars($edit_member_post['additional_text']) : '' ?></textarea><small>Format: "Title: Name - Info, Name - Info"</small></div>
                <div class="form-group"><label>Team Photos:</label><input type="file" name="team_photos[]" multiple accept="image/*"><small>Upload multiple images for carousel. Images will be saved in Mitglieder/ folder.</small></div>
                <?php if (isset($edit_member_post) && $edit_member_post && !empty($edit_member_post['photos'])): ?>
                    <div class="current-images"><strong>Current Photos:</strong><br>
                        <?php $photos = json_decode($edit_member_post['photos'], true); if (is_array($photos)): foreach ($photos as $photo): ?>
                            <div style="display: inline-block; margin:5px;"><img src="<?= htmlspecialchars($photo) ?>" style="max-width:100px;"><br><small><?= basename($photo) ?></small></div>
                        <?php endforeach; endif; ?>
                    </div>
                <?php endif; ?>
        ...
        <div class="section">
            ...
            <table>
                <tr><th>ID</th><th>Year</th><th>Title</th><th>Actions</th></tr>
                <?php foreach ($member_posts as $post): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="POST" style="display:inline;"><input type="hidden" name="member_post_id" value="<?= $post['id'] ?>"><button type="submit" name="delete_member_post" onclick="return confirm('Delete this team year post?')">Delete</button></form>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </table>
        ...
        <div class="section">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <?php if (isset($edit_lesekreis) && $edit_lesekreis): ?>
                    <button type="submit" name="update_lesekreis">Update Lesekreis Post</button><a href="?" class="cancel-btn" style="text-decoration: none; display: inline-block;">Cancel</a>
                <?php else: ?>
                ...
                <?php foreach ($lesekreis_posts as $post): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="POST" style="display:inline;"><input type="hidden" name="lesekreis_id" value="<?= $post['id'] ?>"><button type="submit" name="delete_lesekreis" onclick="return confirm('Delete this Lesekreis post?')">Delete</button></form>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </table>
...
<script>
    ...
    window.addEventListener('load', function() {
        ...
        if (hash && ['events', 'issues', 'press', 'members', 'lesekreis'].includes(hash)) {
            document.querySelectorAll('.tab').forEach(tab => { if (tab.textContent.trim().toLowerCase() === hash) tab.click(); });
        }
        <?php if (isset($_GET['edit_issue'])): ?>showTab('issues');
        <?php elseif (isset($_GET['edit_press'])): ?>showTab('press');
        <?php elseif (isset($_GET['edit_member_post'])): ?>showTab('members');
        <?php elseif (isset($_GET['edit_lesekreis'])): ?>showTab('lesekreis');
        <?php endif; ?>
    });
</script>
</body>
